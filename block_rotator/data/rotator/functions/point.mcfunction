...
execute as @s[scores={RotationInput=1..,RotationPoint=1..}] unless score DisableRails RotationConfig matches 1 if block ~ ~ ~ #minecraft:rails unless block ~ ~-0.5 ~ #minecraft:rails run function rotator:rotate/rails
...
execute if score EnableDebug RotationConfig matches 1 run summon minecraft:area_effect_cloud ~ ~ ~ {Duration:100,Tags:["WrenchRaycast"]}